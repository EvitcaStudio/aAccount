	...
	RegistrationInterface
		...
		TextLabels
			...
			SignUpLabel
				width = 150
				height = 30
				text = '<span style="pointer-events: none; color: #ffffff; font-size: 15px; font-family: Arial;" class="center">Sign up</span>'
...
// ------------------------------------------------------------------------------------------------
		...
		if (pInputCode || pInputCode === 0)
			if (pLabelText)
				text = '<div style="padding-top: 6px; color: #ffffff; font-size: 12px; font-family: Arial;" class="center">' + pLabelText + '</div>'
	...
	command registerCommand()
		...
		if (usernameInput.text && passwordInput.text && repeatPasswordInput.text === passwordInput.text && passwordInput.text.length <= MAX_PASSWORD_LENGTH && passwordInput.text.length >= MIN_PASSWORD_LENGTH && usernameInput.text.length <= MAX_USERNAME_LENGTH && usernameInput.text.length >= MIN_USERNAME_LENGTH && backupInput.text.length === MAX_BACKUP_CODE_LENGTH) // check
			this.sendPacket(aNetwork.S_AACCOUNT_PACKETS.S_CREATE_ACCOUNT_PACKET, [usernameInput.text, passwordInput.text, repeatPasswordInput.text, backupInput.text])
			return
